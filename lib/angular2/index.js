...
module.exports = function generate(ctx) {
  ...
  **/
  ...
  function getUserModelName() {
    let userModelName = '';
    let defaultUserModelExists = false;
    Object.keys(ctx.models).forEach(modelName => {
      if(modelName === 'User') {
        defaultUserModelExists = true;
      }
    ...
    if(!userModelName && defaultUserModelExists) {
      userModelName = 'User';
    }
  ...
  function buildBaseServiceImports(isIo) {
    let imports = [
      { module: 'Injectable, Inject, Optional', from: '@angular/core'},
      { module: 'HttpClient, HttpHeaders, HttpRequest, HttpParams, HttpResponse', from: '@angular/common/http'},
      { module: 'NgModule, ModuleWithProviders', from: '@angular/core'},
      ...
      { module: 'rxjs/add/operator/filter' },
    ];
